...
function createVNFProgrammatically(uuid, vnf_id, vnf_name, vnfx, vnfy, ifaces, edit){ 
	console.log("createVNFProgrammatically vnf_id="+vnf_id+" vnf_name="+vnf_name+" vnfsDefs[vnf_id="+ vnf_id+"]=" + vnfsDefs[vnf_id]);
	...
	console.log("createVNFProgrammatically done: " + instances_vnfs);
	return new_instance_vnf;
}

	
//check that ifaces at nodos[n] has the right naming
function check_ifaces_name(ifaces_ref, ifaces_test){
	var equal = true;
	var iface_ref_dict={};
	var iface_test_dict={};
	var pos;
	for (var pos in ifaces_ref){
		for (var i=0; i<ifaces_ref[pos].length; i++){
			iface_ref_dict[ ifaces_ref[pos][i][0] ] = ifaces_ref[pos][i][1];
		}
	}
	for (pos in ifaces_test){
		for (var i=0; i<ifaces_test[pos].length; i++){
			iface_test_dict[ ifaces_test[pos][i][0] ] = ifaces_test[pos][i][1];
		}
	}
	//compare objects
	for (var item in iface_ref_dict){
		if (! item in iface_test_dict){
			equal = false;
			break;
		}	
		if (iface_ref_dict[item] != iface_test_dict[item]){
			equal = false;
			break;
		}
	...
	if (!equal)
		console.log("iface differents", iface_ref_dict, iface_test_dict);
	return equal;
}
...
function load_scenario(scenario, edit){
	//crear los nodos
	console.log("load_scenario", scenario);
	...
	for(var n in nodos){
		...
		var ifaces = null;
		if (vnf_id==null){
			console.log("Internal error. node with no reference!!!!!!!!!!!!!!!!!!");
			ifaces = {"bottom": [["eth1", "m"]]};
		}else{
			ifaces = vnfsDefs[vnf_id]["ifaces"];
			if ('ifaces' in nodos[n]){
				if (check_ifaces_name(ifaces, nodos[n]['ifaces'] ))
					ifaces = nodos[n]['ifaces'];
			}
		}
		console.log("node load " + n + " name=" + nodos[n]['name'] + " vnf_id=" + vnf_id + " type=" + nodos[n]['type']);
	...
	{
		var net_id = null;
		console.log("connection load type=", connections[c]['type'], connections[c]);
		if (connections[c]['type'] != 'link'){
			...
			var ifaces = null;
			if (net_id==null){
				console.log("Internal error. node net with no reference!!!!!!!!!!!!!!!!!!");
				ifaces = {"bottom": [["eth1", "m"]]};
			}else{
				ifaces = vnfsDefs[net_id]["ifaces"];
				if ('ifaces' in connections[c]){
					if (check_ifaces_name(ifaces, connections[c]['ifaces'] ))
						ifaces = connections[c]['ifaces'];
				}
			} 
			console.log("network node load ", c, "net_id=", net_id, " name=", connections[c]["name"]);
			...
			for(var n2 in connections[c]["nodes"]){
				console.log("net connecting instances_vnfs", instances_vnfs[ connections[c]["nodes"][n2][0] ]);
