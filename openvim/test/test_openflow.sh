...
function delete_and_exit(){
  ...
  do
    ...
    then
      ...
      if [[ $? != 0 ]]
      then 
        echo  "FAIL cannot install old rules: $result" >&2
      else
      ...
      openflow delete  $f -f $debug 
      [[ $? != 0 ]] && echo  "FAIL cannot delete" >&2 || echo OK
    fi
  done
  exit $1
}
...
[[ $? != 0 ]] && echo  "FAIL $result" >&2 && $_exit 1
ports=`echo $result | wc -w`
[[ $ports -lt 4 ]] && echo  "FAIL not enough ports managed by this DPID, needed at least 4" >&2 && $_exit 1
echo OK $ports ports
...
[[ $? != 0 ]] && echo  "FAIL cannot obtain existing rules" >&2 && $_exit 1
echo OK "> ./test_openflow_old_rules.bk"

printf "%-50s" "clearing all current rules: "
openflow clear -f $debug 
[[ $? != 0 ]] && echo  "FAIL cannot clear existing rules" >&2 && $_exit 1
echo OK
TODELETE="restore"
...
[[ $? != 0 ]] && echo  "FAIL cannot insert new rule" >&2 && $_exit 1
...
[[ $? != 0 ]] && echo  "FAIL rule bad inserted" >&2 && $_exit 1
result=`echo $result` #remove blanks
[[ "$result" != "$expected" ]]  && echo -e  "FAIL\n    expected: $expected\n    obtained: $result" >&2 && $_exit 1
echo OK $rule_name
